---
sidebar: sidebar 
permalink: task_subscribe_to_wellness_review_email.html 
keywords: wellness, wellness review subscription, unsubscribe, supportedge expert, supportedge advisor 
summary:  
---
= Digital Advisorからのストレージ システムの健全性に関する通知を購読する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/



NOTE: Wellness Review機能は廃止され、2026年7月をもってDigital Advisorでは利用できなくなりました。

ウェルネスレビューメールに登録すると、ウェルネスステータス、更新日が近づいているストレージシステム、インストール済みベース内の NetApp 製品のアップグレードが必要なストレージシステムをまとめた月次メールを受け取ることができます。ウェルネスレビューメールを受信することで、月次概要を確認し、ストレージシステムに対して必要な対策を講じることができます。

購読しているコンテンツを表示、編集、共有、削除するオプションもあります。いつでも、メールの受信を希望しない場合は、メールの配信停止手続きを行うことができます。

サブスクリプションを有効にしたら、サブスクリプションを追加するときに開始日を選択する必要があります。月ごとの E メールサマリーには、健全性、更新、アップグレード、健全性チェックの未処理のアクションが表示されます。確認用 E メールアドレスを入力すると、指定した E メールアドレスに E メールが送信されます。サブスクリプションを削除することもできます。


NOTE: この機能を使用できるのは、ネットアップの SupportEdge Advisor と SupportEdge Expert のサービスが提供している場合のみです。

.手順
. 左側のペインから「Wellness Review」を選択します。
. 「サブスクリプションを追加」を選択します。
. [ 新規サブスクリプションの追加 ] ダイアログボックスの [ 名前 ] 、 [ カテゴリ * の選択 ] 、 [ 顧客の検索 ] 、および [ 電子メール * ] フィールドに必要な情報を入力します。
. 「Subscribe」を選択します。
+
サブスクリプションが成功すると、「サブスクリプションが追加されました」というメッセージが表示されます。


