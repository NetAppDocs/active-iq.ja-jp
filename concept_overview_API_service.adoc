...
どちらの表示方法でも、左側のナビゲーションペインを使用して利用可能なアイテムを閲覧できます。アイテムはサービスごとにアルファベット順に整理されています。特定のサービス内では、見出しを展開して個々の API エンドポイントを表示できます。サービス見出しまたは API エンドポイントを選択すると、中央ペインのカタログの該当セクションに移動します。
...
Digital Advisor APIへのアクセス方法と使用方法については、link:https://docs.netapp.com/us-en/digital-advisor-automation/["NetApp Digital Advisor APIドキュメント"]を参照してください。
