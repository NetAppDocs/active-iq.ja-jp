...
---
= Digital Advisorで単一および複数のクラスタのONTAPアップグレード プランを生成する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
アップグレード プランを生成する前に、次の操作を行う必要があります。

* AutoSupport（ASUP）が有効化され、正しく設定されており、必要なセクションがすべて含まれていることを確認してください。詳細については、link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/ONTAP_AutoSupport_Resolution_Guide["ONTAP AutoSupport解決ガイド"]を参照してください。
* AutoSupport のクラスタ メンバーが SAP クラスタ構成と一致していることを確認します。詳細については、link:https://kb.netapp.com/Support/NSS/Support_Site/How_to_open_a_non-technical_case["非技術的なケースを開く"] 。
...
.タンイチクラスタ
--
. ダッシュボードで、*Upgrade Advisor* ウィジェットのクラスター数を選択します。
+
...
. アップグレードするクラスターを選択します。*対象OSバージョン*列には、推奨される対象OSバージョンが表示されます。image:edit_icon.png["編集ボタンの画像"]アイコンを選択して、クラスターの別のターゲットOSバージョンを選択できます。
+
image:r_ua_select_target_OS_version_single_cluster.png["ターゲットバージョンの選択方法を示す画像。"]

+
** 「ノード」列でノード数を選択すると、クラスターのノード概要が表示されます。
** アップグレード対象外のクラスターの問題を修正してアップグレード対象にするには、*推奨されるアクション*列から*問題の解決*を選択します。+ image:r_ua_resolve_issue.png["問題のタイプと、それを修正するために必要な解像度を示す画像。"]


. 「アップグレードプランの生成」を選択します。「アップグレード推奨」ページにリダイレクトされます。
...
** image:ua_export_icon.png["[エクスポート]ボタン"]アイコンを選択して、リスクの概要をExcelシートにエクスポートできます。


. 「アップグレード推奨事項」ページから「アップグレードプランの生成」を選択します。
. 表示されたポップアップで詳細を指定します。
  [+]
image:ua_generate_single_clusters_plan.png["アップグレードプランを生成するためのリクエストを送信するための詳細情報を入力するポップアップが表示されます。"]
. 「生成」を選択します。「レポート」ページにリダイレクトされます。
. アップグレードプランは、利用可能になったら*レポート*ページからダウンロードできます。
+

TIP: 「アップグレードアドバイザーレポートを表示」を選択すると、「レポート」ページに移動できます。+ image:r_ua_view_reports.png["View Upgrade Advisorを選択するための位置を示すスクリーンショット。"]
...
. アップグレードするクラスターを選択します。「ターゲット OS バージョン」列には、推奨されるターゲット OS バージョンが表示されます。クラスターの別のターゲット OS バージョンを選択するには、image:edit_icon.png["編集ボタンの画像"]アイコンを選択します。
+
image:r_ua_select_target_OS_version.png["ターゲットバージョンの選択方法を示す画像。"]

+
** 「ノード」列でノード数を選択すると、クラスターのノード概要が表示されます。
** アップグレード対象外のクラスターの問題を修正してアップグレード対象にするには、「推奨されるアクション」列から「問題の解決」を選択してください。+ image:r_ua_resolve_issue.png["問題のタイプと、それを修正するために必要な解像度を示す画像。"]


. 「アップグレードプランの生成」を選択します。
. 表示されたポップアップで詳細を指定します。
  [+]
