...
Active IQ Digital Advisor はDigital Advisorに変更され、ハイブリッド マルチクラウド環境向けの NetApp の統合管理コンソールであるBlueXPに統合されました。link:https://docs.netapp.com/us-en/active-iq/digital-advisor-integration-with-console.html["詳細はこちら。"] 。
...
* * 計画ウィジェット： *
+
** *容量追加：* 既存容量の70%のしきい値を超えるGRIDサイトについては、通知されます。容量しきい値が70%を超える可能性が高い場合、サイト内のStorageGRIDの容量を今後1ヶ月、3ヶ月、6ヶ月間追加するオプションがあります。
