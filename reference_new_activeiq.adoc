...
summary: Digital Advisor の新機能と機能強化について説明します。 
---
= Digital Advisorの新機能
:hardbreaks:
:toclevels: 1
...
Value Insights 機能は、ONTAP システムの全体的な健全性、節約、安定性、およびセキュリティ情報を統合します。この情報を使用して、ONTAP 環境の継続的なデータ主体のビューを通じて、システムの健全性とパフォーマンスを向上させることができます。
...
持続可能性は、E シリーズおよびStorageGRIDシステムでサポートされるようになりました。これらのシステムのサステナビリティ ダッシュボードから、電力、直接的な炭素使用量、熱の予測を表示する推奨アクションと環境指標のリストを表示できます。詳細については、link:https://docs.netapp.com/us-en/active-iq/sustainability-dashboard-overview.html["Sustainabilityダッシュボードの概要"^] 。
...
* ウェルネスワークフローに、顧客、ウォッチリスト、サイト、グループレベルでの推奨アクション数を表示するサステナビリティウィジェットが追加されました。サステナビリティダッシュボードでは、推奨されるアクションの詳細を表示するために、アクションの数を選択できます。詳細については、link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["ストレージシステムの持続可能性を分析する"]を参照してください。
...
* *スナップショットバックアップなしの削減効果*というトグルは、*スナップショットコピーあり*に名称変更され、機能も変更されました。link:https://docs.netapp.com/us-en/active-iq/reference_aiq_faq.html#storage-efficiency["詳細はこちら。"]。
...
持続可能性ダッシュボードは、ストレージ システムの環境的持続可能性に関する貴重な洞察を提供します。持続可能性スコア、炭素削減率、予測される電力使用量、直接炭素、熱などの情報を表示できます。特定のサイトの炭素削減率を調整できます。クラスター レベルで持続可能性スコアを表示することもできます。持続可能性スコアに基づいて、ストレージ システムの全体的な効率を評価し、持続可能性を高めるために NetApp が推奨するアクションと一致させることができます。link:https://docs.netapp.com/us-en/active-iq/learn-about-sustainability.html["詳細はこちら。"] 。
...
SANおよびNASのStorage Efficiencyは、AFF A-Series、AFF C190、All SAN Array、FAS500を含むONTAPシステムのノードレベルで利用可能です。
...
=== NetApp Consoleと統合されたDigital Advisor

Active IQ Digital AdvisorはDigital Advisorに変更され、ハイブリッド マルチクラウド環境向けのNetAppの統合管理コンソールであるNetApp Consoleに統合されました。 link:https://docs.netapp.com/us-en/active-iq/digital-advisor-integration-with-console.html["詳細はこちら。"] 
...
NetApp Consoleユーザーは、Digital Advisorの既存の機能と同様に、該当する場合はDigital Advisorのリンクを新しいタブで開けるようになりました。
...
* ウェルネスレビューのメールには、更新情報ではなく、ランサムウェア対策に関する情報が記載されています。
...
SVMテーブルで青色の「ボリューム概要」ボックスを選択すると、「ポップアウト」が表示され、特定のシリアル番号または物理ノードにホストまたは接続されているボリュームに関する詳細情報が表示されます。
...
* * 計画ウィジェット： *
+
** *容量追加：* 既存容量の70%のしきい値を超えるGRIDサイトについては、通知されます。容量しきい値が70%を超える可能性が高い場合、サイト内のStorageGRIDの容量を今後1か月、3か月、6か月間追加するオプションがあります。
** *更新：* StorageGRIDライセンス契約が既に失効している、または今後6か月以内に失効する予定のシステムについては、通知されます。1つ以上のシステムを選択して、更新のリクエストをNetAppサポートチームに送信できます。
...
* *GRIDビューア：* *設定*ウィジェットから、*GRIDビューア*リンクを選択することで、GRIDの設定を詳細に表示できます。*設定*ウィジェットから、*Grid Viewer*画面の*ダウンロード*ボタンを選択することで、選択したStorageGRIDのサイト詳細と容量詳細をダウンロードできます。
* * サイトの詳細： * このタブには、各サイトで使用可能なグリッドの概要とストレージノードが表示されます。
* *GRID概要:* ライセンスの種類、ライセンス容量、インストールされているノードの数、サポート期間（ライセンス契約の終了日）、プライマリ管理ノード、プライマリ管理ノードのプライマリサイトなどの基本情報が含まれています。このタブには、サイト名と、該当するサイトに紐づけられたストレージノードの数も表示されます。このリリースでは、該当するサイトのストレージノードを表示するために利用可能なハイパーリンクを選択すると、ノード名のリストを表示できます。
...
「ティアデータ」ボタンがFabricPoolダッシュボードに追加されました。このボタンを使用すると、NetApp Consoleを使用して低コストのオブジェクトストレージ階層にデータを階層化できます。
...
Digital Advisor APIを使用して、必要なデータを取得し、会社のワークフローに直接統合することができます。link:https://docs.netapp.com/us-en/active-iq/concept_overview_API_service.html["詳細はこちら。"]。
...
AFFシステム、非AFFシステム、またはその両方について、スナップショットコピーの有無によるストレージシステムのStorage Efficiency比と節約額を確認できます。Storage Efficiencyに関する情報は、ノードレベルで確認できます。link:https://docs.netapp.com/us-en/active-iq/task_analyze_storage_efficiency.html["詳細をご確認ください"]。
